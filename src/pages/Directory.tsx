  ...
  const institutions = [
    {
      ...
      logo: '🏛️',
      description: 'Academic and professional institution contributing to Islamic finance education and research',
      categories: ['Education', 'Research']
    },
    {
      name: 'Islamic Finance Chamber',
      logo: '🏢',
      description: 'A collaborative platform dedicated to promoting sustainable finance practices and education',
      categories: ['Education', 'Research']
    },
    {
      name: 'University Center for Islamic Economics',
      logo: '🎓',
      description: 'An organization focused on increasing the scale and effectiveness of impact investment',
      categories: ['Education', 'Research']
    }
  ]
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants}>
        <h1 className="text-3xl font-bold tracking-tight text-[#000000]">Directory</h1>
        <p className="text-[#737692] mt-2">
          A curated reference hub of institutions, professionals, scholars, and organizations within the Islamic
      ...
      <Tabs defaultValue="institutions" className="w-full">
        ...
        <TabsContent value="institutions" className="mt-6">
          <div className="space-y-8">
                  ...
                  <Card className="hover:shadow-xl transition-all h-full">
                    <CardHeader>
                      <div className="space-y-4">
                        ...
                        <div>
                          <CardTitle className="text-[#000000] mb-2">{institution.name}</CardTitle>
            ...
            {/* Other Institutions */}
            <div>
              <h2 className="text-xl font-bold text-[#000000] mb-6">Other Institutions</h2>
              
              {/* Alphabetical Listing */}
              <Card className="bg-gradient-to-br from-gray-50 to-gray-100">
                <CardHeader>
                  <CardTitle className="text-[#000000]">Other Companies</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid md:grid-cols-3 gap-8">
                    {Object.entries(companiesByLetter).map(([letter, companies]) => (
                      <div key={letter}>
                        <h3 className="text-2xl font-bold text-[#000000] mb-4">{letter}</h3>
        ...
        <TabsContent value="scholars" className="mt-6">
          <div className="grid gap-6 md:grid-cols-3">
            {[
              { name: 'Dr. Muhammad Al-Amin', role: 'Shariah Scholar', organization: 'AAOIFI', expertise: ['Shariah Compliance', 'Islamic Banking'] },
              { name: 'Prof. Fatima Hassan', role: 'Islamic Finance Expert', organization: 'University of Oxford', expertise: ['ESG Finance', 'Research'] },
              { name: 'Sheikh Omar Farooq', role: 'Shariah Advisor', organization: 'IFSB', expertise: ['Sukuk', 'Takaful'] },
              { name: 'Dr. Aisha Rahman', role: 'Islamic Economist', organization: 'Islamic Development Bank', expertise: ['Economics', 'Policy'] },
              { name: 'Dr. Yusuf Ibrahim', role: 'Finance Professor', organization: 'Harvard University', expertise: ['Investment', 'Markets'] },
              { name: 'Sheikh Ahmed Al-Qaradawi', role: 'Shariah Board Member', organization: 'Multiple Institutions', expertise: ['Fiqh', 'Finance'] }
            ].map((scholar, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                ...
                <Card className="hover:shadow-lg transition-all">
                  <CardContent className="p-6">
                    <div className="flex flex-col items-center text-center space-y-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-[#000000]">{scholar.name}</h4>
                        <p className="text-sm text-[#737692]">{scholar.role}</p>
                        <p className="text-xs text-[#737692]">{scholar.organization}</p>
                      </div>
        ...
        <TabsContent value="firms" className="mt-6">
          <div className="grid gap-4">
            {[
              { name: 'Islamic Finance Advisory Group', type: 'Consulting', services: ['Advisory', 'Structuring', 'Compliance'] },
              { name: 'Shariah Audit Solutions', type: 'Auditing', services: ['Shariah Audit', 'Risk Management'] },
              { name: 'Halal Investment Managers', type: 'Asset Management', services: ['Portfolio Management', 'ESG Investing'] },
              { name: 'Islamic Legal Services', type: 'Legal', services: ['Contract Review', 'Regulatory Compliance'] }
            ].map((firm, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-purple-600 to-purple-800 flex items-center justify-center text-white text-lg font-bold">
                          {firm.name.split(' ').map(n => n[0]).slice(0, 2).join('')}
                        </div>
                        <div>
                          <h4 className="font-semibold text-[#000000]">{firm.name}</h4>
        ...
        <TabsContent value="regulatory" className="mt-6">
          <div className="grid gap-6 md:grid-cols-2">
            {[
              ...
              {
                name: 'CIBAFI',
                fullName: 'General Council for Islamic Banks and Financial Institutions',
                location: 'Bahrain',
                ...
                <Card className="hover:shadow-xl transition-all">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-[#000000] text-xl mb-1">{body.name}</CardTitle>
                        <CardDescription className="text-[#737692]">{body.fullName}</CardDescription>
                      </div>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium text-[#000000]">{body.location}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm">
                        <span className="text-[#737692]">Focus Area:</span>
                        <span className="font-medium text-[#000000]">{body.focus}</span>
                      </div>
                      <Button variant="outline" className="w-full border-[#D52B1E] text-[#D52B1E] hover:bg-[#D52B1E] hover:text-white">
                        View Details
                      </Button>
