...
const topics = [
  ...
  { id: 'sustainable-investing', label: 'Sustainable investing' },
  { id: 'data-science', label: 'Data science' },
  { id: 'investment-management', label: 'Investment management' },
  { id: 'private-wealth', label: 'Private wealth' },
  { id: 'investment-performance', label: 'Investment Performance' },
];

export default function InvestmentTopics({ onNext, onBack, initialValue = [] }: InvestmentTopicsProps) {
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12">
        <div className="w-full max-w-4xl">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-3 sm:mb-4 px-4">
            Which of these ethicalt investment<br className="hidden sm:block" />
            <span className="sm:inline"> topics are you most interested in?</span>
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-3xl mx-auto px-4">
            {topics.map((topic) => (
              ...
              >
                ...
                />
                <span className="text-sm sm:text-base text-[#C84A47] font-medium">{topic.label}</span>
              </div>
            ))}
          </div>
        </div>
